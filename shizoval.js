...
// @match            https://client-review-9-public.test-ru.tankionline.com/*
...
class Features {
	...
	weapon = {
		...
		get params() {
			...
			const TargetingSystemWithHorizontalAimAssist = utils.getByProto(targetingSystem, "MAX_VALUE" , 1)

			return {
				TargetingSystemWithHorizontalAimAssist: TargetingSystemWithHorizontalAimAssist,
			};
		},
    ...
    const horizontalAiming = objects.features?.weapon.params.TargetingSystemWithHorizontalAimAssist;


    const horizontalAimingEnabled = utils.findByPath(horizontalAiming, 'i:3')?.[0],
            directionsCount = utils.findByPath(horizontalAiming, 'i:1.i:0')?.[0],
            angleStep = utils.findByPath(horizontalAiming, 'i:1.i:1')?.[0];

    if (objects.features?.weapon.aimBotEnabled) {
        if (horizontalAiming && directionsCount)
        {
            horizontalAiming[horizontalAimingEnabled] = true;
    ...
    {
        horizontalAiming[horizontalAimingEnabled] = false;
...
if (isPhone()) {
  document.addEventListener("touchmove", ({touches}) => {
      if (isDragging && canDrag) {
          console.log(touches[0].clientX, offsetX)
          menu.style.left = `${touches[0].clientX - offsetX}px`;
          menu.style.top = `${touches[0].clientY - offsetY}px`;
      }
  });
  menu.addEventListener("touchstart", ({touches, target}) => {
      if (target.tagName.toLowerCase() === 'input') {
          canDrag = false;
          return;
      }
      canDrag = true, isDragging = true;
      offsetX = touches[0].clientX - menu.getBoundingClientRect().left;
      offsetY = touches[0].clientY - menu.getBoundingClientRect().top;
  });
  document.addEventListener("touchend", () => {
      isDragging = false;
  });
} else {
  menu.addEventListener("mousedown", ({target, clientX, clientY}) => {
      if (target.classList.contains("shizoval_content > input")) {
          canDrag = false;
          return;
      }
      canDrag = true, isDragging = true;
      offsetX = clientX - menu.getBoundingClientRect().left;
      offsetY = clientY - menu.getBoundingClientRect().top;
  });
